...
namespace Revit.SDK.Samples.UIAPI.CS
{
    public partial class PreviewModel : System.Windows.Forms.Form
    {
        ...
        private void updateViewsList(ElementId viewId)
        {
            ...
            {
                ElementType viewType = _dbDocument.GetElement(dbView.GetTypeId()) as ElementType;

                if (viewType.Name.CompareTo("Ceiling Plan") != 0 && viewType.Name.CompareTo("3D View") != 0)
                    continue;
        ...
        private void button1_Click(object sender, EventArgs e)
        {
            ...
            FilteredElementCollector collector = new FilteredElementCollector(_dbDocument).WhereElementIsNotElementType();

            String info = "";
            foreach (Element elem in collector)
            {
                if (null != elem.Category && elem.Category.HasMaterialQuantities)
                {
                    ElementId id = elem.GetTypeId();
                    ElementType type = _dbDocument.GetElement(id) as ElementType;
